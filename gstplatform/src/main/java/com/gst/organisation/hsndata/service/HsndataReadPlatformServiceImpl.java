...
public class HsndataReadPlatformServiceImpl implements HsndataReadPlatformService{
	...
	@Override
	public HsndataData retrieveHsndata(final Long hsnId) {
    	final HsndataMapper rm = new HsndataMapper();
        final String sql = "select " + rm.schema() + " where s.id = ?";
        return this.jdbcTemplate.queryForObject(sql, rm, new Object[] { hsnId });
    }
    ...
    private static final class HsndataMapper implements RowMapper<HsndataData> {
        ...
        public String schema() {
            return " s.id as id, s.hsn_code as hsnCode, s.description as description from g_hsn_data s ";
        }
