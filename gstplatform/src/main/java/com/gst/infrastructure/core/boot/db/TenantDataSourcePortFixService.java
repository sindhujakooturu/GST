 ...
 * under the License.
 */
package com.gst.infrastructure.core.boot.db;
...
public class TenantDataSourcePortFixService {
	...
	if (!enabled)  {
		logger.info("No schema_server_port UPDATE made to tenant_server_connections table of the gst-tenants schema, because " + ENABLED + " = false");
		return;
	}
	if (dsp == null) {
	...
	if ( r == 0 )
		logger.warn("UPDATE tenant_server_connections SET ... did not update ANY rows - something is probably wrong");
			...
			logger.info("Upated "
					+ r
					+ " rows in the tenant_server_connections table of the gst-tenants schema to the real current host: "
